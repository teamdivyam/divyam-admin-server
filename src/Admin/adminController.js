...
import {
  getTotalUser,
  getTotalUserSignupLastXDay,
} from "../services/admin.service.js";
import { getTotalStocks } from "../services/stock.js";
import { getTotalProduct } from "../services/product.service.js";
import { getTotalPackage } from "../services/package.service.js";
...
const GET_ALL_USERS = async (req, res, next) => {
  try {
    ...
    const users = await userModel
      .find({}, { updatedAt: 0, __v: 0, accessToken: 0, createdAt: 0, id: 0 })
      .limit(limit)
      .skip(skip)
      .sort({ createdAt: -1 })
      .lean();
...
const CHANGE_ADMIN_PASSWORD = async (req, res, next) => {
  try {
    const { error, value } = CHANGE_ADMIN_PASSWORD_SCHEMA_VALIDATION.validate(
      req.body
    );
    const reqDATA = value;
    const ADMIN_ID = req?.user;
...
const GET_ORDER_DETAILS = async (req, res, next) => {
  try {
    ...
    if (!Order) {
      return next(
        createHttpError(400, "Internal Error | can't fetch order details ")
      );
    }
...
const getDashboardAnalytics = async (req, res, next) => {
  /**
   * Response JSON:
   * 1. no. of user exists - getTotalUser
   * 2. last 10 day user signup - getTotalUserSignupLastXDay
   * 3. no. of total stocks - getTotalStocks
   * 4. no. of total products - getTotalProduct
   * 5. no. of total packages - getTotalPackage
   * TODO:
   * 6. no. of order booked today - getOrderBookedToday
   * 7. no. of order booked last 10 days - getOrderBookedLastXDay
   * 8.
   */
  try {
    const { LastXDay = 10 } = req.query;
    ...
    const [
      ...
      getTotalUser(),
      getTotalUserSignupLastXDay(LastXDay),
      getTotalStocks(),
      getTotalProduct(),
      getTotalPackage(),
    ]);
    ...
    res.status(200).json({
      success: true,
      message: "return dashboard analytics",
      totalUser: totalUser,
      userSignupLastXDay: userSignupLastXDay,
      totalStock: totalStock,
      totalProduct: totalProduct,
      totalPackage: totalPackage,
      totalOrderBookedToday: 10,
      totalOrderBookedLastXDay: 10,
    });
  } catch (error) {
    console.error("error getting dashboard analytics:", error);
    next(
      createHttpError(500, {
        errorAPI: "GET: admin dashboard anayltics",
